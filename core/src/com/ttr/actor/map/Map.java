...
public class Map extends Group {
	...
	public Map(int width, int height, Level level) {
		...
		for (int row = layout.length - 1; row >= 0; row--) {
			for (int col = 0; col < layout[row].length; col++) {
				if (layout[row][col] == 0) {
					MapTile tile = new FloorTile(layout.length - (1 + row), col, level); // polymorphic for simplicity
																							// in this class
					map[row][col] = tile;
	...
	public void removeBrick(MapTile m) {
		if (!(m instanceof BorderTile)) {
			MapTile n = new FloorTile(layout.length - (1 + m.getRow()), m.getCol(), level);
			addActor(n);
	...
	// tile.setHitbox(tile.getX(), tile.getY(), (float)
	// Math.toRadians(tile.getRotation()));
